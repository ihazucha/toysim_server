...
def plot_roadmarks(roadmarks: np.ndarray, path: np.ndarray):
    ...
    plt.xlabel("X (Road Frame) [m]")
    plt.ylabel("Y (Road Frame) [m]")
...
if __name__ == "__main__":
    reader = RecordReader()
    path_roadframe = record_path("1747773343524591700")
    ...
    config = RoadmarksPlannerConfig(
        roadmark_min_area=50,
        roadmark_max_distance=10,
        roadmark_max_area=10000,
        roadmark_max_count=6,
    )
    ...
    while True:
        ...
        planner.update(img_undistorted)

        plot_img = plot_roadmarks(planner.roadmarks_roadframe, planner.path_roadframe)
        updated_image = draw_debug_data(img_undistorted, planner=planner, camera=camera)
        top_row = np.hstack((img, img_undistorted, planner.img_filtered))
        bottom_row = np.hstack((plot_img, updated_image, updated_image))
